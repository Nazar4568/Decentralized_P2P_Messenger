...
void AppController::setBootstrapHost(const std::string& host)
{
    m_node->setBootstrapHost(host);
}
...
void AppController::changePeerId(const std::string& newPeerId,
                                 const std::string& bootstrapHost,
                                 const std::string& bootstrapPort)
{
    // Tear down the running node, re-configure with the new identity, restart.
    stop();
    configure(newPeerId, m_port);
    start(bootstrapHost, bootstrapPort);
}
...
std::string AppController::bootstrapHost() const
{
    return m_node->bootstrapHost();
}
